...
INTEGER :: lambda1,mu1,lambda2,mu2,lambda3,mu3,epsilon3,Lambda32max,numbLambda3,rhomax,Lambda32,eps3,rho,p1,q1,p2,q2,&
           Lam32,epsilon2max,epsilon3min,Lambda22,Lambda12,Lam32prime,p3,q3,q2min,p1min,pq1,noname1,noname2,&
           Lambda3ind,numb,p3start,q3start,epsilon3mid
REAL(KIND=8) :: N3
REAL(KIND=8),DIMENSION(0:15,0:15,0:15,1:9) :: wignerex
...
!                  .AND.ABS(Lambda12-1)<=Lambda12-1&
              ...
              wignerex(p1,p2,q2,1:rhomax)=DSQRT(DFLOAT((q1+1)*(mu1-q1)*(lambda1+mu1+1-q1)))&
                *DRR3(Lambda22,Lam32prime,Lambda12,1,Lambda12-1,Lam32)*wignerex(p1,p2,q2,1:rhomax)
            ELSE
...
!                  .AND.ABS(Lambda22-Lam32prime)<=Lambda12+1.AND.Lambda12+1<=Lambda22+Lam32prime&
!                  .AND.ABS(Lambda12-1)<=Lambda12+1&
                  )wignerex(p1,p2,q2,1:rhomax)=wignerex(p1,p2,q2,1:rhomax)+DSQRT(DFLOAT((p1+1)*(lambda1-p1)*(mu1+2+p1)))&
              *DRR3(Lambda22,Lam32prime,Lambda12,1,Lambda12+1,Lam32)*wignerex(p1+1,p2,q2,1:rhomax)
!         ELSE
!           wignerex(p1,p2,q2,1:rhomax)=0.D0
!         END IF
...
!                  .AND.ABS(Lambda12-Lambda22-1)<=Lam32prime.AND.Lam32prime<=Lambda12+Lambda22+1&
                  )wignerex(p1,p2,q2,1:rhomax)=wignerex(p1,p2,q2,1:rhomax)+DSQRT(DFLOAT((p2+1)*(lambda2-p2)*(mu2+2+p2)))&
              *DRR3(Lambda12,Lambda22+1,Lam32,1,Lam32prime,Lambda22)*wignerex(p1,p2+1,q2,1:rhomax)
  
            IF(q2/=mu2&
!                  .AND.ABS(Lambda12-Lambda22+1)<=Lam32prime.AND.Lam32prime<=Lambda12+Lambda22-1&
!                  .AND.ABS(Lambda22-2)<=Lambda22&
                  )wignerex(p1,p2,q2,1:rhomax)=wignerex(p1,p2,q2,1:rhomax)+DSQRT(DFLOAT((q2+1)*(mu2-q2)*(lambda2+mu2+1-q2)))&
              *DRR3(Lambda12,Lambda22-1,Lam32,1,Lam32prime,Lambda22)*wignerex(p1,p2,q2+1,1:rhomax)
!         END IF
