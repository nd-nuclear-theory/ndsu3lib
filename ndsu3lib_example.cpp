...
// Simple program calculating SU(3) reduced Wigner and recoupling coefficients and printing them on the screen
{
  ...
  // SU(3)-SU(2)xU(1) reduced Wigner coefficients <(6,1)epsilon1,Lambda1;(2,1)epsilon2,Lambda2||(5,2)-9,5/2>_rho
  {
    ...
    double wigner[dimw];
    ndsu3lib::calculate_wigner_canonical(irrep1, irrep2, irrep3, epsilon3, Lambda3_twice, dimpq, dimw, rhomax, numb, wigner, p1a, p2a, q2a, info);
    if (info == 0)
    {
      ...
      for (int i = 0; i <= numb-1; i++)
      {
        ...
        std::cout << "   " << epsilon1 << "         " << Lambda1_twice << "         " << epsilon2 << "         " << Lambda2_twice << "      ";
  ...
  // SU(3)-SO(3) reduced Wigner coefficients <(6,1)kappa1,2;(2,1)kappa2,3||(5,2)kappa3,3>_rho
  {
    ...
    double wigner[dimen];
    ndsu3lib::calculate_wigner_su3so3(irrep1, L1, irrep2, L2, irrep3, L3, kappa1max, kappa2max, kappa3max, rhomax, dimen, wigner);
    std::cout << std::endl;
    ...
    for(int kappa1 = 1; kappa1 <= kappa1max; kappa1++)
    {
      for(int kappa2 = 1; kappa2 <= kappa2max; kappa2++)
      {
        for(int kappa3 = 1; kappa3 <= kappa3max; kappa3++)
        {
          ...
          for(int rho = 1; rho <= rhomax; rho++)
          {
            int ind = kappa1+kappa1max*(kappa2-1)+kappa1max*kappa2max*(kappa3-1)+kappa1max*kappa2max*kappa3max*(rho-1)-1;
            std::cout << wigner[ind] << "  ";
          }
          std::cout << std::endl;
        }
  ...
  {
    ndsu3lib::su3irrep irrep1 = {9,3}, irrep2 = {1,1}, irrep = {6,6}, irrep3 = {2,2}, irrep12 = {9,3}, irrep23 = {3,3};
    ...
    int dimen = rhomaxa*rhomaxb*rhomaxc*rhomaxd,
        info;
    double rac[dimen];
    ndsu3lib::calculate_u_coeff(irrep1, irrep2, irrep, irrep3, irrep12, irrep23, rhomaxa, rhomaxb, rhomaxc, rhomaxd, dimen, rac, info);
    std::cout << std::endl;
    if(info == 0)
    {
        ...
        for(int rhob = 1; rhob <= rhomaxb; rhob++)
        {
          for(int rhoc = 1; rhoc <= rhomaxc; rhoc++)
          {
            for(int rhod = 1; rhod <= rhomaxd; rhod++)
            {
              ...
              std::cout << "  " << rhoa << "     " << rhob << "     " << rhoc << "     " << rhod << "   " << rac[ind] << std::endl;
            }
          }
        }
      }
  ...
  {
    ndsu3lib::su3irrep irrep2 = {9,3}, irrep1 = {1,1}, irrep = {6,6}, irrep3 = {2,2}, irrep12 = {9,3}, irrep13 = {3,3};
    int rhomaxa = ndsu3lib::outer_multiplicity(irrep1, irrep2, irrep12),
        rhomaxb = ndsu3lib::outer_multiplicity(irrep12, irrep3, irrep),
    ...
    int dimen = rhomaxa*rhomaxb*rhomaxc*rhomaxd,
        info;
    double Zcoeff[dimen];
    ndsu3lib::calculate_z_coeff(irrep2, irrep1, irrep, irrep3, irrep12, irrep13, rhomaxa, rhomaxb, rhomaxc, rhomaxd, dimen, Zcoeff, info);
    std::cout << std::endl;
    if(info == 0)
    {
      ...
      for(int rhoa = 1; rhoa <= rhomaxa; rhoa++)
      {
        for(int rhob = 1; rhob <= rhomaxb; rhob++)
        {
          for(int rhoc = 1; rhoc <= rhomaxc; rhoc++)
          {
            ...
            {
              int ind = rhoa+rhomaxa*(rhob-1)+rhomaxa*rhomaxb*(rhoc-1)+rhomaxa*rhomaxb*rhomaxc*(rhod-1)-1;
              std::cout << "  " << rhoa << "     " << rhob << "     " << rhoc << "     " << rhod << "   " << Zcoeff[ind] << std::endl;
            }
          }
        }
      }
  ...
  //                            |rho13 rho24 rho1234       |
  {
    ...
    int dimen = rhomax12*rhomax34*rhomax1234*rhomax13*rhomax24*rhomax1324,
        info;
    double ninelm[dimen];
    ndsu3lib::calculate_9_lambda_mu(irrep1, irrep2, irrep12, irrep3, irrep4, irrep34, irrep13, irrep24, irrep, rhomax12, rhomax34, rhomax1234, rhomax13, rhomax24, rhomax1324, dimen, ninelm, info);
    std::cout << std::endl;
    if(info == 0)
    {
          ...
          for(int rho1234 = 1; rho1234 <= rhomax1234; rho1234++)
          {
            for(int rho13 = 1; rho13 <= rhomax13; rho13++)
            {
              for(int rho24 = 1; rho24 <= rhomax24; rho24++)
              {
                ...
                {
                  int ind = rho12+rhomax12*(rho34-1)+rhomax12*rhomax34*(rho1234-1)+rhomax12*rhomax34*rhomax1234*(rho13-1)+rhomax12*rhomax34*rhomax1234*rhomax13*(rho24-1)+rhomax12*rhomax34*rhomax1234*rhomax13*rhomax24*(rho1324-1)-1;
                  std::cout << "  " << rho12 << "      " << rho34 << "       " << rho1234 << "       " << rho13 << "      " << rho24 << "       " << rho1324 << "     " << ninelm[ind] << std::endl;
                }
              }
            }
          }
