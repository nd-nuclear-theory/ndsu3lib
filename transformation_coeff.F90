FUNCTION transformation_coeff(I,J,su3irrepx,IEBX,JBT,MBTX,K,L,M) RESULT(DTU3R3)
!-----------------------------------------------------------------------
...
!                 (LSU,03-83)  J.P.DRAAYER        SPACE AND PRECISION   
!                 (LSU,10-84)  J.P.DRAAYER        BINARY TREE TECHNOLOGY
!                 (LSU,01-88)  J.P.DRAAYER        SOON PARK BT CODE     
!                 (LSU,10-89)  J.P.DRAAYER        VS FORTRAN UPGRADE    
!                 (LSU,03-90)  J.P.DRAAYER        REVISED BT LABELING   
!                 (LSU,07-91)  J.P.DRAAYER        CONVERTED BT TO WST   
!                 (UoT,07-95)  C.BAHRI            COMMON BLOCK FOR WST
...
!                                                 STATEMENT FUNCTION "ID" REPLACED BY INTERNAL SUBPROGRAM,
...
!
! This is a new version of DTU3R3
...
REAL(KIND=8) :: DTU3R3,DTS,DAS,DAX,DBS,DS,DX,DCS,DCX
INTEGER :: IEB,MBT,IPH,KBAR,MBAR,IAH,IBH,IPB,IQB,IRB,IW,IX,IY,IS,NTB,MA2,MA1,NA1,NA2,&
           IB1,MB2,MB1,NB1,NB2,IB2,MC2,MC1,NC1,NC2,NC3,NC4,IZMAX,IZMIN,IXMAX,IXMIN,MS2,MS1,&
		   NS1,NS2,IYMAX,IYMIN,ND
TYPE(su3irrep) :: su3irrepy

ND(I,J)=I*(I+1)/2+J+1 ! REPLACEMENT OF THIS STATEMENT FUNCTION WITH AN INTERNAL SUBPROGRAM DOESN'T WORK (J.H. DOESN'T KNOW WHY)
!                                                                       
!     SET HIGHEST WEIGHT/LOWEST WEIGHT                                  
!                                                                       
IF(I/=1)THEN
 su3irrepy%lambda=su3irrepx%mu
 su3irrepy%mu=su3irrepx%lambda
 IEB=-IEBX
 MBT=-MBTX
ELSE
 su3irrepy=su3irrepx
...
END IF
!                                                                       
!     EXTENDED PRECISION OPTION INVOKED IF (LAM+MU+L) > 17              
!                                                                       
IF(su3irrepy%lambda+su3irrepy%mu+L>17)THEN

 DTU3R3=transformation_coeff_quad(1,1,su3irrepy,IEB,JBT,MBT,K,L,M)
...
!                                                                       
 ...
 KBAR=ABS(K)
 MBAR=ABS(M)
 IAH=(su3irrepy%lambda+KBAR)/2
 IBH=(JBT+MBAR)/2
 IPB=(2*(su3irrepy%lambda-su3irrepy%mu)+3*JBT-IEB)/3
 DTS=DTWOS(-IPB)
 IPB=IPB/2
 IQB=su3irrepy%mu+IPB-JBT
 IRB=(JBT+MBT)/2
...
!                                                                       
 DTS=DTS*DFLOAT(IS+1)*&
     DSQRT(DBINO(ND(su3irrepy%lambda,IPB))*DBINO(ND(su3irrepy%mu,IQB))*&
     DBINO(ND(su3irrepy%lambda+su3irrepy%mu+1,IQB))*DBINV(ND(IPB+su3irrepy%mu+1,IQB))*&
     DBINV(ND(JBT,IRB))*DBINO(ND(IS,IX))*DBINV(ND(IS,IY)))
 IF(BTEST(L-IPB+IPH,0))DTS=-DTS
!                                                                       
!     SETUP CONSTANTS                                                   
!                                                                       
 NTB=ID(IPB)
 MA2=su3irrepy%lambda
 MA1=0
 NA1=1
 NA2=ND(MA2,IAH)
 IB1=IRB-MBT
 MB2=IPB
 MB1=su3irrepy%mu-IQB
 NB1=ID(MB1)
 NB2=NTB+IB1
 IB1=IB1+1
 IB2=IPB-IB1-1
 MC2=su3irrepy%lambda+su3irrepy%mu+L
 MC1=IPB+IQB
 NC1=ID(MC1)
...
END IF
!                                                                       
!     ADD OVERALL PHASE FOR (I/=J) CASE                               
!                                                                       
IF(I/=J.AND.BTEST((su3irrepy%lambda+K)/2,0))THEN
 DTU3R3=-DTU3R3
END IF
!                                                                       
!     EXIT CALCULATION                                                  
